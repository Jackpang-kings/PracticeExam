...
namespace HexBaronCS
{
    class Program
    {
        ...
        public static bool CheckUpgradeCommandFormat(List<string> items)
        {
            ...
            if (items.Count == 3)
            {
                if (items[1].ToUpper() != "LESS" && items[1].ToUpper() != "PBDS" && items[1].ToUpper() != "GUARD")
                    return false;
                try
                {
    ...
    class Piece
    {
        ...
        public virtual int SetConnectionsNeededToDestroy()
        {
            return connectionsToDestroy += 1;
        }
    ...
    class LESSPiece : Piece
    {
        ...
        {
            double chances = rNoGen.NextDouble();
            if (terrain != "#")
            {
                return 0;
            }
    ...
    class GuardPiece : Piece
    {
        public GuardPiece(bool player1)
            : base(player1)
        {
            pieceType = "G";
            VPValue = 3;
    ...
    class HexGrid
    {
        ...
        public void AddPiece(bool belongsToPlayer1, string typeOfPiece, int location)
        {
            ...
            else if (typeOfPiece == "Guard") 
            {
                newPiece = new GuardPiece(belongsToPlayer1);
            }
            else
            {
        ...
        private int ExecuteUpgradeCommand(List<string> items, int lumberAvailable)
        {
            ...
            if (!CheckPieceAndTileAreValid(tileToUse) || lumberAvailable < 5 || !(items[1] == "pbds" || items[1] == "less" || items[1] == "guard"))
            {
                return -1;
            }
            else
            {
                ...
                else if (items[1] == "guard")
                {
                    thePiece = new GuardPiece(player1Turn);
                }
                else
                {
                ...
                if (t.GetPieceInTile() != null)
                {
                    ...
                    foreach (var n in listOfNeighbours)
                    {
                        if (n.GetPieceInTile() != null)
                        {
                            ...
                            if (n.GetPieceInTile().GetBelongsToPlayer1() == thePiece.GetBelongsToPlayer1() && n.GetPieceInTile().GetPieceType() == "G")
                            {
                                thePiece.SetConnectionsNeededToDestroy();
                            }
                        }
                    }
